...
namespace Library
{
	...
	const vector<DirectX::XMFLOAT3>& Mesh::Vertices() const
	{
		return mData.Vertices;
	}

	const vector<DirectX::XMFLOAT3>& Mesh::Normals() const
	{
		return mData.Normals;
	}

	const vector<DirectX::XMFLOAT3>& Mesh::Tangents() const
	{
		return mData.Tangents;
	}

	const vector<DirectX::XMFLOAT3>& Mesh::BiNormals() const
	{
		return mData.BiNormals;
	}

	const vector<vector<DirectX::XMFLOAT3>>& Mesh::TextureCoordinates() const
	{
		return mData.TextureCoordinates;
	}

	const vector<vector<DirectX::XMFLOAT4>>& Mesh::VertexColors() const
	{
	...
	void Mesh::CreateIndexBuffer(ID3D12Device9& device, ComPtr<ID3D12Resource> indexBuffer, D3D12_INDEX_BUFFER_VIEW& indexBufferView)
	{
		indexBufferView.BufferLocation = indexBuffer->GetGPUVirtualAddress();
		indexBufferView.Format = DXGI_FORMAT_R32_UINT;
		indexBufferView.SizeInBytes = mData.Indices.size() * sizeof(uint32_t);
		...
		//ThrowIfFailedI(device.CreateBuffer(&indexBufferDesc, &indexSubResourceData, indexBuffer));
	}
	...
	void Mesh::Save(OutputStreamHelper& streamHelper) const
	{
		...
		for (const DirectX::XMFLOAT3& vertex : mData.Vertices)
		{
		...
		for (const DirectX::XMFLOAT3& normal : mData.Normals)
		{
		...
		for (const DirectX::XMFLOAT3& tangents : mData.Tangents)
		{
		...
		for (const DirectX::XMFLOAT3& binormal : mData.BiNormals)
		{
		...
		for (const auto& uvList : mData.TextureCoordinates)
		{
			...
			for (const DirectX::XMFLOAT3& uv : uvList)
			{
		...
		for (const auto& vertexColorList : mData.VertexColors)
		{
			...
			for (const DirectX::XMFLOAT4& vertexColor : vertexColorList)
			{
